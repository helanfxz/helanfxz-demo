...
系统采用轻量 Agent 编排方式，由 `agent/video_generation_workflow.py` 统一串联需求结构化、素材理解、剧本分镜、素材匹配、视频渲染、内容审核和最终检查。每个阶段都会产生结构化 artifact，并把阶段、进度、失败原因和 fallback 信息写入任务状态或报告。

- **模型调用兜底**：文本/多模态模型未配置、超时或返回不可解析时，系统使用规则结构化和商品身份卡兜底，保证商品类型、卖点、主素材、素材风险不会在上游直接丢失。
- **剧本和分镜兜底**：生成的剧本会先停在确认页，用户可以编辑、通过或带意见重新生成；分镜进入渲染前还会做字段补齐、可拍性审核和必要修复，无法确认的任务进入 `needs_review`。
- **渲染兜底**：Seedance 不可用、接口失败或下载失败时，系统会使用本地预览渲染器生成可播放 MP4，并在结果页展示 `fallback_from`，评审仍能体验完整链路。
- **内容审核兜底**：渲染后检查商品漂移、错误 logo、无意义动作、第二商品等问题；可自动修复的分镜会尝试强化身份锚点、简化动作、重写镜头目标或使用本地身份锚定，不能自动修复时进入人工复核。
- **最终状态兜底**：最终检查不通过时不会把任务伪装成完成，而是明确标记为 `needs_review` 或 `failed`，并保留 trace summary 和 report 供复核。
